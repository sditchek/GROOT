                ...
                for drops=1
                    if identconv==1
						...
						% What storms are run in the basin at this init time?   
                            ...
                            if isempty(identdr)==1 && identhwrfmodel==1
                               ...
                               if identhwrfmodel==1;for i=1:size(identdr,2);
                                ...
                                for j=1:size(identexp,1);load('startverif.mat','identconvobstype','identconvobssubtype','identconvobslegend','identconvobscolors');                       
                                    if isempty(identdr)==1;[a,b,c]=unique(identconvobscolors,'rows','stable');identconvlist=1:size(identconvobscolors,1);for ii=1:size(unique(c),1);valstoswitch=identconvlist(c==ii) ;identconvobstype(c==ii)=1000+ii;end;identconvobssubtype=repmat(0,1,size(unique(c),1));identconvobslegend=identconvobslegend(b);identconvobscolors=a;dropaz=NaN;dropan=NaN;xtob=NaN;ytob=NaN;Observation_Class=NaN;droptype0 = NaN;dropsubtype = NaN;droplat = NaN;droplon = NaN;droppres = NaN;dropdhr = NaN;dropinc = NaN;droptype=NaN;if strcmp(tcv_lonew(1),'W')==1;tmpdrops=[-1*droplon droplat droppres dropinc dropaz' dropan' -1*xtob' ytob' dropsubtype repmat(identloop,size(droplat,1),1) droptype];if j==size(identexp,1);RLON(1)=-1*RLON(1);end;else;tmpdrops=[droplon droplat droppres dropinc dropaz' dropan' xtob' ytob' dropsubtype repmat(identloop,size(droplat,1),1) droptype];end;tmpdrops(tmpdrops(:,4)<=0)=NaN;tmpdrops(any(isnan(tmpdrops), 2), :) = [];alldrops01{1}=tmpdrops;else;for i=1:size(identdr,2);
                                    ...
                                    droppres=drops{:,6};
                                    dropdhr=drops{:,7};
                                    dropinc=drops{:,8};dropmult=repmat(1,size(dropinc,1),1);
                                    dropsubtype=drops{:,3};droptype=drops{:,2};elseif identhafsmodel==1;droptype=[];dropsubtype=[];droplat=[];droplon=[];droppres=[];dropdhr=[];dropinc=[];dropmult=[];for filetype=1:size(identdr,1);filename=[identgroot,'obsall/',identexpshort{j},'/',identdr{filetype}];if isfile(filename)==1;ncid = netcdf.open(filename,'NC_NOWRITE');try;droptype00 = single(netcdf.getVar(ncid,2));dropsubtype00 = single(netcdf.getVar(ncid,3)).*identsubtypekeep(filetype); dropc=ismember(droptype00,identconvobstype(filetype));dropd=ismember(dropsubtype00,identconvobssubtype(filetype));drope=dropc+dropd;drope(drope==1)=0;drope(drope==2)=1;drope=logical(drope);droptype0 = single(netcdf.getVar(ncid,2)); droptype0=droptype0(drope);droptype=[droptype;droptype0];dropsubtype0 = single(netcdf.getVar(ncid,3)).*identsubtypekeep(filetype); dropsubtype0=dropsubtype0(drope);dropsubtype=[dropsubtype;dropsubtype0];droplat0 = netcdf.getVar(ncid,4);droplat0=droplat0(drope);droplat=[droplat;droplat0];droplon0 = netcdf.getVar(ncid,5); droplon0=droplon0(drope);droplon0(droplon0>=180)=droplon0(droplon0>=180)-360;droplon0=-1.*droplon0;droplon=[droplon;droplon0];droppres0 = netcdf.getVar(ncid,7); droppres0=droppres0(drope);droppres=[droppres;droppres0];dropdhr0 = netcdf.getVar(ncid,9); dropdhr0=dropdhr0(drope);dropdhr=[dropdhr;dropdhr0];dropinc0 = netcdf.getVar(ncid,netcdf.inqVarID(ncid,'Analysis_Use_Flag')); dropinc0=dropinc0(drope);dropinc=[dropinc;dropinc0];dropmult0=repmat(filetype,size(dropinc0,1),1);dropmult=[dropmult;dropmult0];continue;end;end;end;end;            
									% Add distance away from center at each pressure level
                                    if isnan(droplat)==1
                                        dropaz=NaN;
                                        dropan=NaN;
                                        xtob=NaN;
                                        ytob=NaN;dropmult=NaN;
                                    elseif isempty(droplat)==1;dropaz=NaN;dropan=NaN;xtob=NaN;ytob=NaN;dropmult=NaN;Observation_Class=NaN;droptype0 = NaN;dropsubtype = NaN;droplat = NaN;droplon = NaN;droppres = NaN;dropdhr = NaN;dropinc = NaN;droptype=NaN;else;
                                    ...
                                    % Make Full List
                                    if strcmp(tcv_lonew(i),'W')==1
                                        tmpdrops=[-1*droplon droplat droppres dropinc dropaz' dropan' -1*xtob' ytob' dropsubtype repmat(identloop,size(droplat,1),1) droptype dropmult];
                                        if j==size(identexp,1)
                                            RLON(i)=-1*RLON(i);
                                        end
                                    else
                                        tmpdrops=[droplon droplat droppres dropinc dropaz' dropan' xtob' ytob' dropsubtype repmat(identloop,size(droplat,1),1) droptype dropmult]; %% STOPPED HERE - MUST ADD IN COLUMN FOR DROPTYPE! ENSURE IT WORKS WITH HWRF TOO!
                                    end
                                    tmpdrops(tmpdrops(:,4)<=0)=NaN;
                                    tmpdrops(any(isnan(tmpdrops), 2), :) = [];                
									end;[a,b,c]=unique(identconvobscolors,'rows','stable');identconvlist=1:size(identconvobscolors,1);for ii=1:size(unique(c),1);valstoswitch=identconvlist(c==ii) ;tmpdrops(ismember(tmpdrops(:,12),valstoswitch),11)=1000+ii;identconvobstype(c==ii)=1000+ii;end;identconvobssubtype=repmat(0,1,size(unique(c),1));identconvobslegend=identconvobslegend(b);identconvobscolors=a;tmpdrops=tmpdrops(:,1:11);identconvobstype=unique(identconvobstype);
                                ...
                                if identgraphicsconv==1
                                    ...
                                    for loop1=1:size(identexp,1)
                                        ...
                                        if size(identconvobssubtype,2)>1 && strcmp(pwd,'/scratch4/AOML/aoml-hafs1/Sarah.D.Ditchek/GROOT/GROOT-AOMLMV')==0
                                            for sot=1:size(identconvobssubtype,2)
                                        ...
                                        elseif size(identconvobssubtype,2)>1 && strcmp(pwd,'/scratch4/AOML/aoml-hafs1/Sarah.D.Ditchek/GROOT/GROOT-AOMLMV')==1;lsz=numdrops;else;
                        ...
                        for exl=1:size(identexp,1)            
                            ...
                            for cyc=1:size(alldropscomp,2)
                                ...
                                for i=1:size(tmp2_all,2)
                                    ...
                                    tmp3_all=tmp2_all{i};identdropsbycycle(cyc,exl)=size(tmp3_all,1);
                            ...
                            for loopdrops=1 % loop over all drops                
                                ...
                                elseif size(identconvobssubtype,2)>1 && strcmp(pwd,'/scratch4/AOML/aoml-hafs1/Sarah.D.Ditchek/GROOT/GROOT-AOMLMV')==1;lsz=size(tmp1,1);else;
                        ...
                        if size(identconvobssubtype,2)>1
                            ...
                            save([identout,'RESULTS/',identfold,'VERIFICATION/OBS/',upper(identhwrf),identn(end-1:end),'_conv.mat'],'identdroplist','identdropcyc','valuescomp','denmapcomp','denmapcomp_t','listcomp','identdropsbycycle','-v7.3')
                        else
                            identdroplist={identn identinnerdrops identgalesdrops identouterdrops};
                            save([identout,'RESULTS/',identfold,'VERIFICATION/OBS/',upper(identhwrf),identn(end-1:end),'_conv.mat'],'identdroplist','identdropcyc','valuescomp','denmapcomp','listcomp','identdropsbycycle','-v7.3')
                        end            
                        close all
                        %% Track with Obs
                        for track=1:size(identexp,1)
                            ...
                            if size(identconvobssubtype,2)>1 && strcmp(pwd,'/scratch4/AOML/aoml-hafs1/Sarah.D.Ditchek/GROOT/GROOT-AOMLMV')==0% there are subtypes to this conventional observation!
                                for sot=1:size(identconvobssubtype,2)
