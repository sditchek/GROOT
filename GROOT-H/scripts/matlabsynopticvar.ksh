...
#SBATCH -J SYNVAR	    # XXXX: Job Name | change to whatever you'd like
...
# Find how many variables were selected
typeset -a arr
cnt=0
...
then
	...
	then
		...
		if [ "${initgraphicssynoptic}" -eq 1 ]
		then
			matlab -nosplash -nodesktop -r "identbatch=${numarrc};identvar0=${numarrv};identvarexp0=${numarre};identda=0;identgraphics=0;identstorm=0;identsynoptic=0;identstormrt=0;identstormav=0;identsynopticrt=0;identsynopticav=0;identgraphicsstorm=0;identgraphicssynoptic=${initgraphicssynoptic};identshear=0;identsynopticclean=0;identstormclean=0;" < ${scriptspath}/rungrb.m > ${outputpath}/OUTPUT_SYNOPTIC_GRAPHICS_${stm}_${numarrc}_${numarrv}_${numarre}.txt &
			wait
		fi
		numexp=`expr $numexp + 1`
		if [ $numexp -lt $lenexp ]
	        then
		    sbatch ${scriptspath}/matlabsynopticvar_${numvar}_${numexp}.ksh $numvar $numexp $scriptspath $outputpath $homepath $numcyc $numarrc
		else
			...
			then
				echo $numvar
				sbatch ${scriptspath}/matlabsynopticvar_${numvar}_0.ksh $numvar 0 $scriptspath $outputpath $homepath $numcyc $numarrc
			else
				rm ${scriptspath}/matlabsynopticvar_*.ksh
				...
				if [ $numcyc -lt $lencyc ]
				then
				matlab -nosplash -nodesktop -r "identbatch=${numarrc};identvar0=0;identvarexp0=0;identda=0;identgraphics=0;identstorm=0;identsynoptic=0;identstormrt=0;identstormav=0;identsynopticrt=0;identsynopticav=0;identgraphicsstorm=0;identgraphicssynoptic=0;identshear=0;identsynopticclean=1;identstormclean=0;" < ${scriptspath}/rungrb.m > ${outputpath}/OUTPUT_SYNOPTIC_GRAPHICS_${stm}_${i}_CLEAN.txt &
					wait
					sbatch ${scriptspath}/matlabsynopticbatch_$numcyc.ksh $numcyc $scriptspath $outputpath $homepath
				
				else
					matlab -nosplash -nodesktop -r "identbatch=${numarrc};identvar0=0;identvarexp0=0;identda=0;identgraphics=0;identstorm=0;identsynoptic=0;identstormrt=0;identstormav=0;identsynopticrt=0;identsynopticav=0;identgraphicsstorm=0;identgraphicssynoptic=0;identshear=1;identsynopticclean=1;identstormclean=0;" < ${scriptspath}/rungrb.m > ${outputpath}/OUTPUT_SYNOPTIC_GRAPHICS_${stm}_${i}_CLEAN.txt &
                		        wait	
					sbatch ${scriptspath}/matlabsynopticclean.ksh $scriptspath $homepath
				fi
			fi
		fi
	fi
