	...
	for identloop=1:size(identinittimesunique,1)
		...
		if exist('initnum','var')==0 %% to take care of invests not in BT
			...
			for tmp=1:size(identexp,1)                
				filename = dir([identgroovpr,identexpshort{tmp},'/',identhwrf,'.',identinittimesunique(identloop,:),'*']);filename=[identgroovpr,identexpshort{tmp},'/',filename.name];
				[identhemi,DATEall,BASINall,NAMEall,CATall,LATall,POall,SE50all,LONall,PRESSall,SE64all,NE34all,RAD34all,SPEEDall,NE50all,RAD50all,SW34all,NE64all,RAD64all,SW50all,NW34all,RMWall,SW64all,NW50all,ROall,NW64all,SE34all,FHRall,INTCHall,LANDall]=atcf(filename,0);
			...
			if identgraphicsbycycle==1
				...
				for plt_trk=1
						...
						f = getframe(hfig);
						filename=[identtrackint,'/FULL/trkcomp_',identinittimesunique(identloop,:)];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
						close all 
				end
				% Plot MaxSpd
				for plt_spd=1
					...
					set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, .72, 0.96]); % maximize figure window
					f = getframe(hfig);
					filename=[identtrackint,'/FULL/spdcomp_',identinittimesunique(identloop,:)];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
					close all 
				end
				...
				for plt_trk=1
					for pltcomp=1:18 
					   ...
					   elseif pltcomp==10
						...
						elseif pltcomp==17
							tmp_name='ate';
							tmp_title='Along-Track Bias Comparison (km)';
							tmp_bt=nan(1,22);
							tmp_exp=squeeze(ateerr_exp(identloop,1:skip:end,:));
							ylabel('Bias (km)','fontsize',20)            
							ylim([-500 500])
						elseif pltcomp==18
							tmp_name='xte';
							tmp_title='Across-Track Bias Comparison (km)';
							tmp_bt=nan(1,22);
						...
						set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, .72, 0.96]); % maximize figure window
						f = getframe(hfig);
						filename=[identtrackint,'/FULL/',tmp_name,'comp_',identinittimesunique(identloop,:)];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
						close all                        
					end
				end
			else
	...
	if strcmp(identcycles,'all')==0
		...
		if identcompositeonly==0
			...
			for fhr=1:size(identexp,1)
				...
				set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, .72, 0.96]); % maximize figure window
				f = getframe(hfig);
				filename=[identtrackint,'/FULL/spdcomp_all_',identexp{fhr}];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
				close all 
			end
			% All Else
			for plt=1:18
				for fhr=1:size(identexp,1)
				   ...
				   if plt==1
					...
					elseif plt==17
						...
						tmp_name='atecomp';
						tmp_title='Along-Track Bias Comparision (km)';
						tmp_ylab='Bias (km)';
						ylim([-500 500])  
					elseif plt==18
						...
						tmp_bt=nan(size(BT_rmw(:,1:skip:end)));
						tmp_name='xtecomp';
						tmp_title='Across-Track Bias Comparision (km)';
						tmp_ylab='Bias (km)';
						ylim([-500 500])  
				   elseif plt==19
						tmp_exp=cat(1,EXP_ne34(:,1:skip:end,:),EXP_nw34(:,1:skip:end,:),EXP_se34(:,1:skip:end,:),EXP_sw34(:,1:skip:end,:));
				   ...
				   if plt>=17 && plt <=18
					...
					set(gca,'Color',[.9 .9 .9])
					f = getframe(hfig);
					filename=[identtrackint,'/FULL/',tmp_name,'_all_',identexp{fhr}];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
					close all 
				end
			end
		end
	...
	if identcompositeonly==0
		...
		for plt=[1:18,21:23]                                                  
			...
			for identexploop=1:size(tmp_exp,3)	
				...
				f = getframe(hfig);
				filename=[identtrackint,'/FULL/',identn,'_',tmp_name,'_LAGCORR_',identexpshort{identexploop}];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
				close all													
			end
		end 
...
%				end
			...
			f = getframe(hfig);
			filename=[identtrackint,'/FULL/',identn,'_',tmp_name];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
			close all
		end                    
		...
		for med=1:2; for plt=[1:18,21:23]
			...
			elseif plt==3
			 ...
			 elseif plt==4
				tmp_exp=ne34err_exp(:,1:skip:end,:);
				tmp_name='neR34errskill';
				tmp_title='R34 NEQ Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			elseif plt==5
				tmp_exp=se34err_exp(:,1:skip:end,:);
				tmp_name='seR34errskill';
				tmp_title='R34 SEQ Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			elseif plt==6
				tmp_exp=sw34err_exp(:,1:skip:end,:);
				tmp_name='swR34errskill';
				tmp_title='R34 SWQ Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			elseif plt==7
				tmp_exp=nw34err_exp(:,1:skip:end,:);
				tmp_name='nwR34errskill';
				tmp_title='R34 NWQ Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			elseif plt==8
				tmp_exp=ne50err_exp(:,1:skip:end,:);
				tmp_name='neR50errskill';
				tmp_title='R50 NEQ Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			elseif plt==9
				tmp_exp=se50err_exp(:,1:skip:end,:);
				tmp_name='seR50errskill';
				tmp_title='R50 SEQ Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			elseif plt==10
				tmp_exp=sw50err_exp(:,1:skip:end,:);
				tmp_name='swR50errskill';
				tmp_title='R50 SWQ Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			elseif plt==11
				tmp_exp=nw50err_exp(:,1:skip:end,:);
				tmp_name='nwR50errskill';
				tmp_title='R50 NWQ Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			elseif plt==12
				tmp_exp=ne64err_exp(:,1:skip:end,:);
				tmp_name='neR64errskill';
				tmp_title='R64 NEQ Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			elseif plt==13
				tmp_exp=se64err_exp(:,1:skip:end,:);
				tmp_name='seR64errskill';
				tmp_title='R64 SEQ Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			elseif plt==14
				tmp_exp=sw64err_exp(:,1:skip:end,:);
				tmp_name='swR64errskill';
				tmp_title='R64 SWQ Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			elseif plt==15
				tmp_exp=nw64err_exp(:,1:skip:end,:);
				tmp_name='nwR64errskill';
			...
			elseif plt==21
				tmp_exp=cat(1,ne34err_exp(:,1:skip:end,:),nw34err_exp(:,1:skip:end,:),se34err_exp(:,1:skip:end,:),sw34err_exp(:,1:skip:end,:));
				tmp_name='R34errskill';
				tmp_title='R34 Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			elseif plt==22
				tmp_exp=cat(1,ne50err_exp(:,1:skip:end,:),nw50err_exp(:,1:skip:end,:),se50err_exp(:,1:skip:end,:),sw50err_exp(:,1:skip:end,:));
				tmp_name='R50errskill';
				tmp_title='R50 Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			elseif plt==23
				tmp_exp=cat(1,ne64err_exp(:,1:skip:end,:),nw64err_exp(:,1:skip:end,:),se64err_exp(:,1:skip:end,:),sw64err_exp(:,1:skip:end,:));
				tmp_name='R64errskill';
				tmp_title='R64 Skill (\%)';
				tmp_ytitle=['Skill wrt ', identexpsigimpshort,' (%)'];
			end
			if plt <19 || plt >20
		...
		for plt=[1:18,21:23] % no across or along for bias
			...
			for identexploop=tmpu
				...
				f = getframe(hfig);
				filename=[identtrackint,'/FULL/',identn,'_',tmp_name,'_contr_',identexpshort{identexploop}];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
				close all   
			end 														
		end  																							
		% Create PPC Graphics
		for plt=[1:18,21:23] % no across or along for bias
			...
			for identexploop=tmpu
				...
				set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, .9, 0.96]); % maximize figure window
				f = getframe(hfig);
				filename=[identtrackint,'/FULL/',identn,'_',tmp_name,'_PPC_',identexpshort{identexploop}];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
		...
		for scorecard=1
			...
			for identexploop=tmpu % loop over experiments   
				...
				elseif plt==9
					...
					%if sum(isnan(ttestsc(abs(tmp_exp(:,:,identexploop)),abs(tmp_exp(:,:,tmpimp)),squeeze(scfactor(plt0,identexploop,:))','alpha',.05)'))==size(tmp_exp,2);sigtest_95(:,plt)=repmat(0,1,size(tmp_exp,2)); else sigtest_95(:,plt)=ttestsc(abs(tmp_exp(:,:,identexploop)),abs(tmp_exp(:,:,tmpimp)),squeeze(scfactor(plt0,identexploop,:))','alpha',.05)'; end
				...
				% Specify y labels
				tmp_ytitle={'TRACK','VMAX','PMIN','R34','R50','R64','RMW'}';
				...
				for vari=1:7
					...
					b=find((tmp_imp(:,vari)>=1 & tmp_fsp(:,vari)>=((0.5.*tmp_fcst(:,vari)+max(5,0.01.*tmp_fcst(:,vari)))./tmp_fcst(:,vari)*100-50) & tmp_impmed(:,vari)>-1) | (tmp_imp(:,vari)>=1 & tmp_fsp(:,vari)>-1 & tmp_impmed(:,vari)>=1) | (tmp_imp(:,vari)>-1 & tmp_fsp(:,vari)>=((0.5.*tmp_fcst(:,vari)+max(5,0.01.*tmp_fcst(:,vari)))./tmp_fcst(:,vari)*100-50) & tmp_impmed(:,vari)>=1)); % MC-imp
					...
					d=find((tmp_imp(:,vari)<=-1 & tmp_fsp(:,vari)<=-((0.5.*tmp_fcst(:,vari)+max(5,0.01.*tmp_fcst(:,vari)))./tmp_fcst(:,vari)*100-50) & tmp_impmed(:,vari)<1) | (tmp_imp(:,vari)<=-1 & tmp_fsp(:,vari)<1 & tmp_impmed(:,vari)<=-1) | (tmp_imp(:,vari)<1 & tmp_fsp(:,vari)<=-((0.5.*tmp_fcst(:,vari)+max(5,0.01.*tmp_fcst(:,vari)))./tmp_fcst(:,vari)*100-50) & tmp_impmed(:,vari)<=-1)); % MC-deg
				...
				text(-2.1,29.5,['\textbf{R34}'],'color','w','rotation',90,'HorizontalAlignment','center','VerticalAlignment','middle','fontsize',12,'fontweight','bold','interpreter','latex')
				text(-2.1,37.5,['\textbf{R50}'],'color','w','rotation',90,'HorizontalAlignment','center','VerticalAlignment','middle','fontsize',12,'fontweight','bold','interpreter','latex')
				text(-2.1,45.5,['\textbf{R64}'],'color','w','rotation',90,'HorizontalAlignment','center','VerticalAlignment','middle','fontsize',12,'fontweight','bold','interpreter','latex')
		...
		for plt=1:23
			...
			xL=xlim;
			yL=ylim;
			ylabel({'# lead','times'},'Position',[xL(2) yL(2)],'VerticalAlignment','top','HorizontalAlignment','center','Rotation',0); 
			...
			filename=[identtrackint,'/FULL/',identn,'_',tmp_name,'_cycles'];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
			close all
		end
				...
				for strat=stratlist    
					...
					if exist('breakstrat','var')==1 && identcompositeonly==0
							...
							for plt=1:23
								...
								filename=[identtrackint,'/',identn,'_',tmp_name,'_',stname];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
								close all
							end   
							% Create Graphics: trk, int, spd Skill vs. deny
							for plt=[1:18,21:23]
							...
							for plt=1:23
								...
								axis ij;b=axes('Position',[a1Pos(1) a1Pos(2)-.11 a1Pos(3) a1Pos(4)]);yyaxis left;set(b,'YColor','none');yyaxis right;set(b,'plotboxaspectratio',[1 1 1]);set(b,'Color','none');set(b,'XColor','none');set(b,'tickdir','both');set(b,'YColor',[0 0 0]);set(b,'xtick',[]);ylim(b,[xstart-.5 size(BT_lat,1)+.5]);tmpxend=size(BT_lat,1);if size(identinittimesunique,1)>50;set(b,'ytick',[1:4:100]); else;set(b,'ytick',[1:2:100]); end ;trkerr_exp_fcnt=zeros(1,tmpxend);trkerr_exp_fcnt1=zeros(1,tmpxend);trkerr_exp_fcnt2=zeros(1,tmpxend);clear cntexp;for tmp=1:size(identexp,1);cntexp(:,tmp)=sum(~isnan(tmp_exp(:,:,tmp)),2);end;set(gca,'fontsize',12);xL=xlim;yL=ylim;ylabel({'# lead','times'},'Position',[xL(2) yL(2)],'VerticalAlignment','top','HorizontalAlignment','center','Rotation',0); set(gca,'position',[spPos(1)+.08 spPos(2)+.05 spPos(3) spPos(4)]);screenposition = get(gcf,'Position');set(gcf,'PaperPosition',[0 0 screenposition(4) screenposition(4)],'PaperSize',[screenposition(4) screenposition(4)]);set(gcf, 'InvertHardcopy', 'off');maxmin=squeeze(sum(~isnan(tmp_exp(:,:,:)),2)); if size(identinittimesunique,1)>50;maxmin=maxmin(1:4:end,:); else;maxmin=maxmin(1:2:end,:); end;yTicks0 = get(gca, 'yticklabel');yTicks = get(gca, 'ytick');set(b,'yticklabels',[]);minX =1;HorizontalOffset1=0.03;VerticalOffset = 0.2;maxmin=maxmin.*0;for xx = 1:size(yTicks0,1);tmptxt0=maxmin(xx,:);tmptxt1=[];if size(unique(tmptxt0),2)==1;tmptxt1=num2str(tmptxt0(1));else;for xxx=1:size(maxmin,2);  if xxx==size(maxmin,2);	  tmptxt1=[tmptxt1,'\color[rgb]{',num2str(identexpcolors(xxx,:)),'}',num2str(tmptxt0(xxx))];  else;tmptxt1=[tmptxt1,'\color[rgb]{',num2str(identexpcolors(xxx,:)),'}',num2str(tmptxt0(xxx)),'\color[rgb]{0,0,0}/'];  end;end;end;text(minX+HorizontalOffset1,yTicks(xx),tmptxt1,'verticalalignment','middle','fontsize',12);end;axis ij;f = getframe(hfig);
								filename=[identtrackint,'/',identn,'_',tmp_name,'_',stname,'_cycles'];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
								close all
							end   
							% Create Graphics: trk, int, spd Skill vs. deny
							for plt=[1:18,21:23]
							...
							for plt=[1:18,21:23] % no across or along for bias
								...
								for identexploop=tmpu
									...
									set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, .9, 0.96]); % maximize figure window
									f = getframe(hfig);
									filename=[identtrackint,'/',identn,'_',tmp_name,'_contr_',stname,'_',identexpshort{identexploop}];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
									close all   
								end 														
							end  																							
							% Create PPC Graphics
							for plt=[1:18,21:23] % no across or along for bias
								...
								for identexploop=tmpu
									...
									set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, .9, 0.96]); % maximize figure window
									f = getframe(hfig);
									filename=[identtrackint,'/',identn,'_',tmp_name,'_PPC_',stname,'_',identexpshort{identexploop}];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;					
									%print([identtrackint,'/trackcomp_fhr_',num2str((fhr-1)*3),'h'],'-dpdf','-r200');
									close all   
								end 
							end                             
...
%							end
						...
						elseif identcompositeonly==0
							...
							for med=1:2; for plt=[1:18,21:23]
								...
								set(cm, 'layer', 'top');[uca,ucb]=max(ultcon);if size(find(ultcon==uca),2)==1;else;ultconmean(find(ultcon~=uca))=NaN;[ucaa,ucbb]=max(ultconmean);if sum(ucbb==find(ultcon==uca))>0;ucb=ucbb;end;end;for uuu=tmpu;axes(cm(uuu));set(gca,'Clipping','Off');uch1=plot(0:.1:.5,[1 1 1 1 1 1 ],'-','color',identexpcolors(uuu,:),'linewidth',7-cmlocc);uch2=plot(23.5:.1:24,[1 1 1 1 1 1 ],'-','color',identexpcolors(uuu,:),'linewidth',7-cmlocc);uistack(uch1,'bottom');uistack(uch2,'bottom');end;axes(ax1);if sum([cntexp(:)])==0;tmpstr='Not Enough Data';elseif size(identexp,1)<3;tmpstr=['HDCI: \color[rgb]{',num2str(identexpcolors(ucb,:)),'}',identexpshort{ucb}];else;[ucc,ucd]=min(ultcon);if size(find(ultcon==ucc),2)==1;else;ultconmean(find(ultcon~=ucc))=NaN;[uccc,ucdd]=min(ultconmean);if sum(ucdd==find(ultcon==ucc))>0;ucd=ucdd;end;end;tmpstr=['HDCI: \color[rgb]{',num2str(identexpcolors(ucb,:)),'}',identexpshort{ucb},' \color[rgb]{0,0,0}| LDCI: \color[rgb]{',num2str(identexpcolors(ucd,:)),'}',identexpshort{ucd}];end;end;if size(identexp,1)<3;else;if med==1;tx=text(0.99,0.1,tmpstr,'HorizontalAlignment','right','VerticalAlignment','top','BackgroundColor', 'w','EdgeColor','k','fontsize',12,'units','normalized');set(tx, 'Layer', 'front');end;end;f = getframe(hfig);if med==1;filename=[identtrackint,'/',identn,'_',tmp_name,'_',stname,'_mean'];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;elseif med==2; filename=[identtrackint,'/',identn,'_',tmp_name,'_',stname,'_median'];if identeps==1;set(gcf,'PaperPositionMode','auto');print([filename,'.eps'],'-depsc','-r0');else;imwrite(f.cdata,[filename,'.png'],'png');end;end;
								close all
							end; end;																	
							...
							for scorecard=1
								...
								for identexploop=tmpu % loop over experiments   
									...
									for vari=1:7
										...
										sconsistent(vari,a)=2;
										b=find((tmp_imp(:,vari)>=1 & tmp_fsp(:,vari)>=((0.5.*tmp_fcst(:,vari)+max(5,0.01.*tmp_fcst(:,vari)))./tmp_fcst(:,vari)*100-50) & tmp_impmed(:,vari)>-1) | (tmp_imp(:,vari)>=1 & tmp_fsp(:,vari)>-1 & tmp_impmed(:,vari)>=1) | (tmp_imp(:,vari)>-1 & tmp_fsp(:,vari)>=((0.5.*tmp_fcst(:,vari)+max(5,0.01.*tmp_fcst(:,vari)))./tmp_fcst(:,vari)*100-50) & tmp_impmed(:,vari)>=1));
										[~,~,ind]  = intersect(a,b);
										...
										d=find((tmp_imp(:,vari)<=-1 & tmp_fsp(:,vari)<=-((0.5.*tmp_fcst(:,vari)+max(5,0.01.*tmp_fcst(:,vari)))./tmp_fcst(:,vari)*100-50) & tmp_impmed(:,vari)<1) | (tmp_imp(:,vari)<=-1 & tmp_fsp(:,vari)<1 & tmp_impmed(:,vari)<=-1) | (tmp_imp(:,vari)<1 & tmp_fsp(:,vari)<=-((0.5.*tmp_fcst(:,vari)+max(5,0.01.*tmp_fcst(:,vari)))./tmp_fcst(:,vari)*100-50) & tmp_impmed(:,vari)<=-1));
										[~,~,ind]  = intersect(c,d);
