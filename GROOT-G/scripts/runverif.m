...
for identremoveinvest=1
    ...
    if identindivstorm==1
		...
		stormsdone{stmdn}=identhwrf;end; %GROOT-G Change End
				...
				identdr=dir([identgroot,identexp{tmp},'/atcf/',identtmp1,'*','atcfunix']); %GROOT-G Change
    ...
    %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %%
    if identcomposite==1
      for stmdn=1:size(stormsdone,2)   %GROOT-G Change Start
            %% %%%%%%%%%%%%%%%% %%
            %% Set Up the Storm %%
            %% %%%%%%%%%%%%%%%% %%
        ...
        if identcompositeprep==1
			...
			for basinloop=1:size(identdr5,2) 
					...
					if identconv==1 || identsatobs==1;stratlist=[stratlist 895:910];end;
		...
		if identcompositerun==1
			...
			if size(stormsdone,2)>1 && strcmp(identdr5{1},'AL')==1 || strcmp(identdr5{1},'EP')==1 || strcmp(identdr5{1},'CP')==1 || strcmp(identdr5{1},'WP')==1 || strcmp(identdr5{1},'IO')==1 || strcmp(identdr5{1},'SH')==1 
				...
				for basinloop=1:size(identdr5,2) 
					...
					if isempty(identdr)==1
						...
						if basinloop==1
							...
							BT_name_all     =   {};
							BT_target_all   =   [];
							BT_drops_all    =   [];BT_enkf_all=[];BT_hfip_all=[];BT_obsg_all=[];
							BT_year_all    =   [];
							BT_date_all     =   [];
							BT_storm_all    =   [];
							BT_lat_all      =   [];
							...
							ateerr_exp_all	=	[];
							xteerr_exp_all	=	[];
							interr_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							ne34err_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							ne50err_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							ne64err_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							nw34err_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							nw50err_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							nw64err_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							poerr_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							rmwerr_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							roerr_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							se34err_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							se50err_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							se64err_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							spderr_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							sw34err_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							sw50err_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							sw64err_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							trkerr_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							ateerr_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							xteerr_exp_stm = nan(2000,identmodelfhr,size(identexp,1),size(identdr,2));
							cntcan=1;
							cntst=1;
						end
				...
				BT_date=BT_date_all;
				BT_name=BT_name_all;
				BT_drops=BT_drops_all;BT_hfip=BT_hfip_all;if identenkfexact==1 | identenkfoper==1;BT_enkf=BT_enkf_all;end;BT_obsg=BT_obsg_all;
			...
			for compositetcs=1 %% included cycles
				...
				for basinloop=1:size(identdr5,2)
					...
					if isempty(identdr)==1
						...
						nm_stm1=nan(200,1,size(tmpnm,2));
						nm_stm2=nan(200,1,size(tmpnm,2));
						nm_stm3=nan(200,1,size(tmpnm,2));									
