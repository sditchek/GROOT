...
identout=['/scratch4/AOML/aoml-hafs1/Sarah.D.Ditchek/GROOT/GROOT-G/'];  	 % top-level directory path for results folder - must include end "/"
identgroot=['/scratch4/AOML/aoml-hafs1/Sarah.D.Ditchek/GROOT/GROOT-G/GROOT-PR/'];% path of GROOT-PR directory (i.e., ${homepath}/GROOT/GROOT-G/GROOT-PR/, where homepath is same as in runverif.ksh) - must include end "/"
identmaxfhr=126;			               		             	 % the maximum forecast lead time you want in your graphics 
...
identcasename={'13AL'};tmpcasefold='LAURA20';identcasebasin='LAURA13L';identbasinid='AL';    % identcasename=ID and basin identifier (e.g., 05AL) | tmpcasefold=uppercase name and 2-digit year (e.g., DORIAN19) | identcasebasin=uppercase name, ID, and single-letter identifier for the basin (e.g., 05L) | identbasinid=uppercase 2-letter basin identifier
...
parent_dir = identgroot; subdirs = dir(parent_dir);largest_values = [];for i = 1:size(subdirs,1)-2;if subdirs(i).isdir && ~ismember(subdirs(i).name,{'.', '..'});subdir_path = fullfile(parent_dir, subdirs(i).name);files = dir(fullfile(subdir_path, '*')); for j = 3:size(files,1);file_path = fullfile(subdir_path, files(j).name);fid = fopen(file_path, 'rt');if fid == -1;warning('Could not open file: %s', file_path);continue;end;lines = textscan(fid, '%s', 'Delimiter', '\n');fclose(fid);last_line = lines{1}{end}; columns = str2double(strsplit(last_line));extracted_values = columns(6);largest_values = [largest_values, (extracted_values)]; end;end;end;identmodelfhr=max(largest_values);
